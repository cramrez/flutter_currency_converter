...
class CurrencyRepository extends CurrencyRepositoryBase {
  ...
  Stream<List<Currency>> getCurrencies() async* {
    ...
    final symbols = await _restProvider.symbols();

    final currenciesList = currencies.entries.map((it) {
      final name = symbols[it.key]!;
      return Currency(it.key, name, it.value, timestamp);
    }).toList();

    await Future.forEach<Currency>(
        currenciesList, (it) async => await _dbProvider.insert(it));
  ...
  Future<void> enableCurrency(String key, int position) =>
      _dbProvider.enableCurrency(key, position);
  ...
  Future<void> setSelectedCurrency(String key) =>
      _dbProvider.setSelectedCurrency(key);
}
